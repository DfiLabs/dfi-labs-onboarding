...
export const handler = async (event: any) => {
  try {
    ...
    try {
      ...
      // Try alternative path - maybe it's stored differently
      try {
        console.log(`Trying alternative path: screening/${caseId}/results.json`)
        const altResponse = await s3.send(new GetObjectCommand({
          Bucket: BUCKET,
          Key: `screening/${caseId}/results.json`
        }))
        ...
        if (altData) {
          ...
          // For testing purposes, if we don't have an email, use a default one
          if (!clientEmail) {
            clientEmail = 'louis.benassy@gmail.com' // Default email for testing
        ...
        // If all else fails, use default email for testing
        if (!clientEmail) {
...
<body>
  <div class="container">
    ...
    // Return a nice HTML page instead of ugly JSON
    const successPage = `
<!DOCTYPE html>
<html>
<head>
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <p style="color: #e0e0e0; text-align: center;">
        ${action === 'approve' ? 'The client has been notified of their account approval.' : 
          action === 'reject' ? 'The client has been notified of their account rejection.' : 
          'The client has been notified that additional information is required.'}
      </p>
    </div>
